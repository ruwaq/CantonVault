...
public class LedgerApi {
    ...
    public LedgerApi(LedgerConfig ledgerConfig, Optional<TokenProvider> tokenProvider) {
        ...
        // Conditional TLS: plaintext for dev/LocalNet, transport security for prod
        if (ledgerConfig.isTlsEnabled()) {
            builder.useTransportSecurity();
        } else {
            builder.usePlaintext();
        }

        // Keep-alive: prevent idle connections from being dropped, detect dead peers
        ...
        logger.atInfo()
                ...
                .addKeyValue("tls", ledgerConfig.isTlsEnabled())
                .log("Connected to ledger");

        // Stubs with deadline — prevents indefinite hangs on unresponsive ledger
    ...
    public <T extends Template> CompletableFuture<Void> create(
        ...
        return traceWithStartEvent(ctx, () -> {
            ...
            return submitCommands(List.of(command.build()), commandId, List.of(), actAsParty).thenApply(submitResponse -> null);
        });
    }
    ...
    public CompletableFuture<CommandSubmissionServiceOuterClass.SubmitResponse> submitCommands(
        ...
        return submitCommands(cmds, commandId, List.of(), actAsParty);
    }
