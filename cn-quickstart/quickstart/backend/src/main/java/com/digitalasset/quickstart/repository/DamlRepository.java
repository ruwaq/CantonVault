...
public class DamlRepository {
    ...
    // ── CantonVault Vault queries ────────────────────────────────────────────

    public CompletableFuture<List<Contract<CommitmentProposal>>> findActiveCommitmentProposals() {
        return pqs.active(CommitmentProposal.class);
    }

    public CompletableFuture<List<Contract<CommitmentProposal>>> findCommitmentProposalsForParty(String party) {
        return pqs.activeWhere(CommitmentProposal.class,
                "payload->>'proposer' = ? OR payload->>'accepter' = ?", party, party);
    }

    public CompletableFuture<Optional<Contract<CommitmentProposal>>> findCommitmentProposalById(String contractId) {
    ...
    public CompletableFuture<List<Contract<CommitmentContract>>> findCommitmentsForParty(String party) {
        return pqs.activeWhere(CommitmentContract.class,
                "payload->>'proposer' = ? OR payload->>'accepter' = ?", party, party);
    }
    ...
    public CompletableFuture<List<Contract<DisputeCase>>> findDisputeCasesForParty(String party) {
        return pqs.activeWhere(DisputeCase.class,
                "payload->>'proposer' = ? OR payload->>'accepter' = ? OR payload->>'thirdParty' = ?",
                party, party, party);
    }

    public CompletableFuture<List<Contract<SettlementReceipt>>> findSettlementReceipts() {
        return pqs.active(SettlementReceipt.class);
    }

    public CompletableFuture<List<Contract<SettlementReceipt>>> findSettlementReceiptsForParty(String party) {
        return pqs.activeWhere(SettlementReceipt.class,
                "payload->>'proposer' = ? OR payload->>'accepter' = ?", party, party);
    }

    public CompletableFuture<List<Contract<DisclosedRecord>>> findDisclosedRecordsForObserver(String party) {
        return pqs.activeWhere(DisclosedRecord.class,
                "payload->>'observer' = ?", party);
    }

    public CompletableFuture<List<Contract<DisclosedRecord>>> findDisclosedRecords() {
        return pqs.active(DisclosedRecord.class);
    }
}
